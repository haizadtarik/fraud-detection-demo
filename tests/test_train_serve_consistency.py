...
def test_stateless_features_match():
    ...
    txn = Transaction(
        transaction_type="TRANSFER", amount=50000.0, name_orig="C111", name_dest="M222", step=14
    )
...
def test_hour_derivation_matches():
    ...
    for step in [0, 5, 6, 13, 23, 24, 47, 100]:
        ...
        train_row = _training_features_for_one(raw, params)
        txn = Transaction(
            transaction_type="CASH_OUT", amount=100.0, name_orig="C1", name_dest="C2", step=step
        )
