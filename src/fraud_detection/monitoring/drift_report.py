...
def main():
    ...
    log.info(
        f"Reference: {len(reference):,} rows (fraud {reference[TARGET].mean():.3%}) | "
    ...
    if len(flagged):
        log.warning(f"{len(flagged)} feature(s) drifting: {', '.join(flagged.feature)}")

    # --- Target drift (the prevalence jump) ---
    tgt_psi = binary_psi(reference[TARGET], current[TARGET])
    log.info(
        f"Target (fraud-rate) PSI: {tgt_psi:.4f} "
    ...
    try:
        ...
        schema = DataDefinition(
            numerical_columns=[
                ...
                if c not in ("is_transfer", "is_night", "dest_is_merchant", "dest_is_new")
            ],
            categorical_columns=[
                "is_transfer",
                "is_night",
        ...
        log.warning(
            f"Evidently report failed ({type(e).__name__}: {e}). "
