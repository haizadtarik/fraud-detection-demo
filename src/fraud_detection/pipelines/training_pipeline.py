...
from fraud_detection.models.evaluate import (
    core_metrics, precision_at_k, cost_optimal_threshold,
)
...
def main(source_label: str):
    ...
    with mlflow.start_run(run_name=f"lgbm_{source_label}"):
        ...
        log.info(f"ROC-AUC={m['roc_auc']:.4f} | PR-AUC={m['pr_auc']:.4f} "
                 f"| test prevalence={m['prevalence']:.3%}")
        log.info(f"Cost-optimal threshold={thr['threshold']:.4f} -> "
                 f"precision={thr['precision']:.3f}, recall={thr['recall']:.3f}, "
                 f"FP={thr['fp']}, FN={thr['fn']}")
        for k in params["evaluate"]["precision_at_k"]:
            kk = min(k, len(y_te))
            log.info(f"  precision@{kk}={m[f'precision_at_{kk}']:.3f} "
                     f"recall@{kk}={m[f'recall_at_{kk}']:.3f}")

        mlflow.log_metrics(m)
        mlflow.log_metrics({f"thr_{k}": v for k, v in thr.items()
                            if isinstance(v, (int, float))})

        # --- calibration (reliability) ---
        frac_pos, mean_pred = calibration_curve(y_te, y_score,
                                                n_bins=params["evaluate"]["calibration_bins"])
        log.info("Calibration (mean_pred -> observed_freq): " +
        ...
        joblib.dump({"model": model, "features": FEATURE_COLUMNS,
                     "threshold": thr["threshold"]}, out)
